...
class MCPClient {
  ...
  async callStorefrontTool(toolName, toolArgs) {
    try {
      const normalizedToolArgs = this._normalizeStorefrontToolArgs(toolName, toolArgs);

      console.log("Calling storefront tool", toolName, normalizedToolArgs);
  ...
  /**
   * Normalizes storefront tool arguments when the model emits a flattened shape.
   *
   * Shopify's search_catalog schema expects query/filter/page data under
   * arguments.catalog, but models sometimes emit { query: "..." } directly.
   * Passing the flattened shape returns a generic catalog page instead of
   * search results, so normalize it before making the MCP call.
   *
   * @private
   * @param {string} toolName - Name of the storefront tool
   * @param {Object} toolArgs - Arguments generated by the model
   * @returns {Object} Normalized arguments
   */
  _normalizeStorefrontToolArgs(toolName, toolArgs = {}) {
    if (toolName !== "search_catalog" || toolArgs.catalog) {
      return toolArgs;
    }

    const catalog = {};

    ["query", "context", "signals", "filters", "pagination"].forEach((key) => {
      if (toolArgs[key] !== undefined) {
        catalog[key] = toolArgs[key];
      }
    });
    ...
    const { query, context, signals, filters, pagination, ...rest } = toolArgs;
